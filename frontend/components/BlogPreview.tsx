...
function renderBlog(text: string) {
  return text.split(/\n\n+/).map((para, i) => {
    ...
    return (
      <p
        key={i}
        className="text-[#9A9080] leading-relaxed text-sm mb-4 last:mb-0"
    ...
    <div className="space-y-4">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {!review.approved && review.correction_note && (
            <span className="text-[10px] text-red-400/60 hidden sm:block">
      ...
      {!review.approved && review.correction_note && (
        <div className="border-l-2 border-red-900/60 pl-4 py-2 bg-red-950/10">
          ...
          <p className="text-xs text-red-400/80">{review.correction_note}</p>
        </div>
      )}
      ...
      {!review.approved && review.issues.length > 0 && (
        <div className="border border-red-900/30 bg-red-950/10 p-3">
          <div className="text-[9px] tracking-widest uppercase text-red-500 mb-2">
            Editor Issues
          </div>
      ...
      <div className="border border-[#141414] p-8">
        <div className="max-w-prose mx-auto">
          <h1
            className="text-xl font-bold text-[#F5F0E8] mb-2 leading-tight"
