      ...
      >
        ...
        >
          ...
          {!review.approved && review.correction_note && (
            <span className="font-normal ml-2 hidden sm:inline text-red-400/60">
          ...
          <div className="w-72 border-2 border-[#2a2520] rounded-3xl overflow-hidden bg-[#0a0a0a] shadow-2xl">
            ...
            <div className="overflow-y-auto max-h-96">
              {content.thread.map((post, i) => (
                <div
                  key={i}
                  className={`px-4 py-3 border-b border-[#0e0e0e] flex gap-2.5 ${
                    i === 0 ? "bg-[#0e0e0e]" : ""
                  }`}
                >
                  <div className="w-7 h-7 rounded-full border border-[#C9A84C]/30 flex items-center justify-center mt-0.5">
                    <span className="text-[8px] text-[#C9A84C]/70">CP</span>
                  </div>

                  <div className="flex-1">
                    <div className="flex items-center gap-1 mb-0.5">
                      <span className="text-[9px] font-bold text-[#F5F0E8]">
                        CampaignPilot
                      </span>
                      <span className="text-[9px] text-[#4A4540]">· now</span>
                    </div>

                    <p className="text-[10px] text-[#9A9080] leading-snug">
                      {post}
                    </p>

                    <div className="flex gap-3 mt-1.5">
                      {["💬", "🔁", "❤️"].map((ic, j) => (
                        <span key={j} className="text-[9px] text-[#2a2520]">
                          {ic}
                        </span>
                      ))}
                    </div>
            ...
            <div key={i} className="flex gap-3">
              <div className="flex flex-col items-center">
                <div className="w-8 h-8 border border-[#C9A84C]/30 rotate-45 flex items-center justify-center">
                  <span className="text-[#C9A84C] text-[9px] font-bold rotate-[-45deg]">
                    CP
                  </span>
                </div>
                {i < content.thread.length - 1 && (
                  <div className="w-px bg-[#1a1a1a] flex-1 my-1" />
                )}
              </div>
              ...
              <div className="flex-1 pb-3">
                ...
                <div className="flex gap-4 mt-2">
                  {["◻ Reply", "⇄ Share", "♡ Like"].map((a, j) => (
                    <span key={j} className="text-[10px] text-[#2a2520]">
                      {a}
                    </span>
                  ))}
                </div>
