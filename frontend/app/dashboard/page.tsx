...
import { updateCampaignInDB } from "@/lib/db-api"; // ← NEW

type Tab = "blog" | "thread" | "email" | "facts" | "review" | "export" | "assistant";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const dbId    = sessionStorage.getItem("dbCampaignId");   // ← NEW
    ...
    if (t)    setTone(t);
    if (dbId) setDbCampaignId(dbId);  // ← NEW
  ...
  const handleRegenerate = async (channel: Channel) => {
    ...
    try {
      ...
      // ← NEW: update the campaign in MongoDB after regeneration
      ...
      if (dbCampaignId && dbUserId) {
        try {
          ...
          console.error("[Dashboard] Failed to update campaign in DB:", dbErr);
          // Non-fatal — the UI still shows the correct updated content
        }
      }
    } catch (e: unknown) {
            ...
            <div className="flex gap-0 mb-4 border border-[#1a1a1a] bg-[#0a0a0a] p-1 overflow-x-auto">
              {tabs.map(tab => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className={`flex items-center gap-1.5 px-4 py-2 text-[10px] tracking-widest uppercase flex-shrink-0 transition-all ${activeTab === tab.id ? "bg-[#C9A84C]/10 text-[#C9A84C] border border-[#C9A84C]/20" : "text-[#4A4540] hover:text-[#9A9080]"}`}>
                  <span>{tab.icon}</span>
